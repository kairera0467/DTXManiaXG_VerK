...
・[共通]アニメーション補助システムにWindowsAnimationManagerを導入。(SharpDX.Animation)
・[XG]一部画像を大幅作り直し。
...
2020092800	4.10a	.NET Frameworkを4.5.1から4.7.1に変更。
					matixx風演奏画面の一部テクスチャ追加
					全レーンオートプレイの場合、グラフの表示をしないよう変更。
					matixx風選曲画面で「5_header panel.png」が無いと落ちる不具合を修正。
					CConfigIniのSoundDeviceTypeの説明文をWASAPI共有を含めたものに更新。
					DTXMania Rev:0c282febd430a19882761187d8704f9829cc36c7の修正(64bit動作時にMIDI機器が使用できない不具合修正)を適用
					XG風演奏画面のネームカラー表示を少し修正。
					全オート時、リザルト画面でEランク文字のテクスチャが解放漏れを起こす不具合を修正。
					matixx風演奏画面でギター演奏画面に入れなかったので修正。
					Matixx風表示でGuitar演奏をすると1ノーツ目の判定文字が表示された瞬間に強制終了する不具合の修正。
