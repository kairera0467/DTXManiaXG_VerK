...
namespace DTXMania
{
	internal class CStageタイトル : CStage
	{
		...
		public override void OnManagedリソースの作成()
		{
			if ( !base.b活性化してない )
			{
				this.tx背景 = CDTXMania.tテクスチャの生成( CSkin.Path( @"Graphics\2_background.png" ), false );
		...
		public override int On進行描画()
		{
			if ( !base.b活性化してない )
			{
				...
				#endregion
                #region[ バージョン表示 ]
                string strVersion = "DTX:J:A:A:2017052000";
#if DEBUG
                strVersion += "  DEBUG";
#endif
		...
		private int MENU_H = 39;
		private int MENU_W = 227;
		private int MENU_X = 506;
		private int MENU_Y = 513;
