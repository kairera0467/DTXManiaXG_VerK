...
namespace DTXMania
{
	internal class CActSelectステータスパネルXG : CActSelectステータスパネル共通
	{
		...
		public CActSelectステータスパネルXG()
		{
			base.b活性化してない = true;
		}
		public override void t選択曲が変更された()
		{
			C曲リストノード c曲リストノード = CDTXMania.stage選曲XG.r現在選択中の曲;
			Cスコア cスコア = CDTXMania.stage選曲XG.r現在選択中のスコア;
		...
		public override int On進行描画()
		{
			if( !base.b活性化してない )
			{
				...
				#region [ パネル本体の描画 ]
				//-----------------
				if( this.txパネル本体 != null && CDTXMania.stage選曲XG.act曲リスト.ct登場アニメ用[5] != null )
				{
                    {
                ...
                int n難易度ラベル合計値1P = 0;
                int n難易度ラベル合計値2P = 0;
                if( this.tx難易度パネル != null && CDTXMania.stage選曲XG.act曲リスト.ct登場アニメ用[5] != null )
                {
                    for (int i = 0; i < 5; i++)
                    {
                    ...
                    if( CDTXMania.ConfigIni.bDrums有効 )
                    {
                        if( CDTXMania.stage選曲XG.act曲リスト.ct登場アニメ用[5].n現在の値 == 100 )
                        {
                            ...
                            if( n難易度ラベル合計値 == 0 && CDTXMania.stage選曲XG.r現在選択中の曲.eノード種別 == C曲リストノード.Eノード種別.SCORE ) //難易度ラベルが存在しなかった場合は5に描画する。
                        ...
                        else
                        {
                            for( int i = 0; i < 5; i++ )
                            {
                                ...
                                mat *= Matrix.RotationX( 1.60f - (float)( (float)CDTXMania.stage選曲XG.act曲リスト.ct登場アニメ用[ 5 ].n現在の値 * 0.016f ) );
                    ...
                    //                this.tx難易度パネル.t2D描画(CDTXMania.app.Device, this.n本体X[2] + (143 * i), this.n本体Y[2] - y差分[i], new Rectangle(x差分[i], 0 + (98 * i), 132, 98));
                            ...
                            for( int i = 0; i < 2; i++ )
                            {
                                if( ( i == 0 ? n難易度ラベル合計値1P == 0 : n難易度ラベル合計値2P == 1 ) && CDTXMania.stage選曲XG.r現在選択中の曲.eノード種別 == C曲リストノード.Eノード種別.SCORE ) //難易度ラベルが存在しなかった場合は5に描画する。
                ...
                if( CDTXMania.stage選曲XG.act曲リスト.ct登場アニメ用[ 5 ] != null )
                {
                    if( CDTXMania.stage選曲XG.act曲リスト.ct登場アニメ用[ 5 ].b終了値に達した )
                    {
                        #region [ 難易度文字列の描画 ]
                        //-----------------
                        for( int i = 0; i < 5; i++ )
	    			    ...
	    			    {
                            if( CDTXMania.r現在のステージ.eフェーズID == CStage.Eフェーズ.共通_通常状態 )
                                this.tx難易度数字XG.n透明度 = (int)( CDTXMania.stage選曲XG.act曲リスト.ct登場アニメ用[ 5 ].n現在の値 * 2.6f );
                            for( int j = 0; j < 3; j++ )
                            {
                        ...
                        if( CDTXMania.r現在のステージ.eフェーズID == CStage.Eフェーズ.選曲_決定演出 || CDTXMania.r現在のステージ.eフェーズID == CStage.Eフェーズ.選曲_NowLoading画面へのフェードアウト )
                        {
                            ...
                            else if( this.tx決定後_難易度パネル1P != null && CDTXMania.stage選曲XG.r確定されたスコア.譜面情報.b譜面がある.Drums )
                            {
                                //挙動が謎のためY軸回転は保留。
        ...
        private void t決定アニメ_難易度パネルを生成する()
        {
            ...
            if( CDTXMania.ConfigIni.bDrums有効 && imgPanel != null )
            {
                ...
                //XG譜面
                if( !CDTXMania.stage選曲XG.r確定されたスコア.譜面情報.b完全にCLASSIC譜面である.Drums )
                {
                    #region[ XG譜面 ]
                    int n難易度整数;
                    int n難易度小数;
