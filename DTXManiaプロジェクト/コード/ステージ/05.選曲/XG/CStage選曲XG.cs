...
namespace DTXMania
{
    internal class CStage選曲XG : CStage選曲
    {
        public CStage選曲XG()
        {
            base.list子Activities.Add( this.act曲リスト = new CActSelect曲リストXG() );
            base.list子Activities.Add( this.actステータスパネル = new CActSelectステータスパネルXG() );
            base.list子Activities.Add( this.actShowCurrentPosition = new CActSelectShowCurrentPositionXG() );
        }
        ...
        public override int On進行描画()
		{
			if( !base.b活性化してない )
			{
                ...
                {
                    if( this.ct決定演出待機.n現在の値 < 1000 )
                    {
                        this.tx背景_決定後.n透明度 = (int)( 255 * ( ( this.ct決定演出待機.n現在の値 ) / 500.0 ) );
                    }
                    else if( this.ct決定演出待機.n現在の値 > 1000 )
                    {
					...
					if ( !this.actSortSongs.bIsActivePopupMenu && !this.actQuickConfig.bIsActivePopupMenu )
					{
						...
						*/
						...
						if ( this.act曲リスト.r現在選択中の曲 != null )
						{
							...
							#endregion
							#region [ Up ]
							this.ctキー反復用.Up.tキー反復( CDTXMania.Input管理.Keyboard.bキーが押されている( (int) SlimDXKey.LeftArrow ), new CCounter.DGキー処理( this.tカーソルを上へ移動する ) );
							...
							#endregion
							#region [ Down ]
							this.ctキー反復用.Down.tキー反復( CDTXMania.Input管理.Keyboard.bキーが押されている( (int) SlimDXKey.RightArrow ), new CCounter.DGキー処理( this.tカーソルを下へ移動する ) );
							...
							if ( ( ( this.act曲リスト.r現在選択中の曲 != null ) && ( this.act曲リスト.r現在選択中の曲.r親ノード != null ) ) &&
								( CDTXMania.Pad.b押された( E楽器パート.DRUMS, Eパッド.LC ) ||
								  ...
								  CDTXMania.Input管理.Keyboard.bキーが押された( (int)SlimDXKey.UpArrow ) ) )
							{
								this.actPresound.tサウンド停止();
								CDTXMania.Skin.sound取消音.t再生する();
