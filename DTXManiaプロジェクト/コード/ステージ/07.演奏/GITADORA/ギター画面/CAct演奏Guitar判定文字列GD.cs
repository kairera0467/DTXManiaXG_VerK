                ...
                for( int j = 0; j < 15; j++ )
                {
                    if (!base.st状態[j].ct進行.b停止中)
                    {
                        ...
                        if( CDTXMania.ConfigIni.eJudgeAnimeType != Eタイプ.C )
                        {
                            ...
                            int nJudgePosY = CDTXMania.stage演奏ギター画面GITADORA.演奏判定ライン座標.n判定ラインY座標( E楽器パート.DRUMS, false, CDTXMania.ConfigIni.bReverse.Drums, false, true );
                            ...
                            if( j == 14 )
                            {
                                ...
                                num6 = CDTXMania.ConfigIni.bReverse.Bass ?
                                    ( ( ( CDTXMania.ConfigIni.判定文字表示位置.Bass == E判定文字表示位置.レーン上 ) ? 240 : 100) + ( this.n文字の縦表示位置[ j ] * 0x20 ) ) :
                                    ( ( ( CDTXMania.ConfigIni.判定文字表示位置.Bass == E判定文字表示位置.レーン上 ) ? 180 : 300) + ( this.n文字の縦表示位置[ j ] * 0x20 ) );
                            }
                            else if( j == 13 )
                            {
                                ...
                                if( !( CDTXMania.DTX.bチップがある.Bass || ( CDTXMania.ConfigIni.判定文字表示位置.Guitar == E判定文字表示位置.レーン上 ) ) )
                                {
                                    num5 = 0x198;
                                    num6 = 0x93;
                                }
                                ...
                                {
                                    num5 = this.stレーンサイズ[ j ].x;
                                    num6 = CDTXMania.ConfigIni.bReverse.Guitar ?
                                        ( ( ( CDTXMania.ConfigIni.判定文字表示位置.Guitar == E判定文字表示位置.レーン上 ) ? 240 : 100 ) + ( this.n文字の縦表示位置[ j ] * 0x20 ) ) :
                                        ( ( ( CDTXMania.ConfigIni.判定文字表示位置.Guitar == E判定文字表示位置.レーン上 ) ? 180 : 300 ) + ( this.n文字の縦表示位置[ j ] * 0x20 ) );
                                }
                            }
                        ...
                        else if( CDTXMania.ConfigIni.eJudgeAnimeType == Eタイプ.C )
                        {
                            ...
                            int num5 = 0;
                            int num6 = 0;
                            int nJudgePosY = CDTXMania.stage演奏ギター画面GITADORA.演奏判定ライン座標.n判定ラインY座標( j == 13 ? E楽器パート.GUITAR : E楽器パート.BASS, false, CDTXMania.ConfigIni.bReverse.Drums, false, true );
                            base.iP_A = nJudgePosY - 189;
                            base.iP_B = nJudgePosY + 23;
							if( j == 13 )
							{
								if( CDTXMania.ConfigIni.判定文字表示位置.Guitar == E判定文字表示位置.表示OFF )
									continue;
                                num5 = 186;
                                num6 = CDTXMania.ConfigIni.bReverse.Guitar ? 450 : 320;
                                switch( CDTXMania.ConfigIni.判定文字表示位置.Guitar )
                                {
                                    case E判定文字表示位置.判定ライン上:
                                        num6 = CDTXMania.ConfigIni.bReverse.Guitar ? 670 : 100;
                                        break;
                                    case E判定文字表示位置.コンボ下:
                                        num5 = 426;
                                        num6 = 310;
							...
							else if( j == 14 )
							{
								if( CDTXMania.ConfigIni.判定文字表示位置.Bass == E判定文字表示位置.表示OFF )
									continue;
                                num5 = 1052;
                                num6 = CDTXMania.ConfigIni.bReverse.Bass ? 450 : 320;
                                switch( CDTXMania.ConfigIni.判定文字表示位置.Bass )
                                {
                                    case E判定文字表示位置.判定ライン上:
                                        num6 = CDTXMania.ConfigIni.bReverse.Bass ? 670 : 100;
                                        break;
                                    case E判定文字表示位置.コンボ下:
                                        num5 = 865;
                                        num6 = 310;
